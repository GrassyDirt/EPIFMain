...
function siteseo_pro_disable_manual_update_for_plugin($transient){
	...
	$pro_plugin = 'siteseo-pro/siteseo-pro.php';
	...
	if(!empty($GLOBALS['siteseo_pro_is_upgraded'])){
		$pro_version = siteseo_pro_file_get_version_num($pro_plugin);
	}
	...
	if(!empty($pro_version) && version_compare($free_version, $pro_version, '<')){
		...
		$transient->response[$plugin]->package = 'https://downloads.wordpress.org/plugin/siteseo.'.$pro_version.'.zip';
	}else{
		unset($transient->response[$plugin]);
	}
