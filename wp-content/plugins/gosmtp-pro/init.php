...
if(!defined('SITEPAD')){
	add_filter('site_transient_update_plugins', 'gosmtp_pro_disable_manual_update_for_plugin', 99);
	add_filter('pre_site_transient_update_plugins', 'gosmtp_pro_disable_manual_update_for_plugin', 99);
	add_filter('transient_update_plugins', 'gosmtp_pro_disable_manual_update_for_plugin', 99);
...
function gosmtp_pro_load_plugin(){
	...
	// Register GoSMTP PRO abilities with the WordPress 6.9+ Abilities API.
	if(class_exists('\GOSMTP\AbilitiesPro') && function_exists('wp_register_ability')){
		if(!empty($gosmtp->options['ai_abilities']) && !empty($gosmtp->options['ai_abilities']['enabled'])){
			add_action('wp_abilities_api_categories_init', '\GOSMTP\AbilitiesPro::register_categories');
			add_action('wp_abilities_api_init', '\GOSMTP\AbilitiesPro::register_abilities');
		}

		add_filter('gosmtp_registered_abilities', 'GOSMTP\AbilitiesPro::abilities');
	}
}
